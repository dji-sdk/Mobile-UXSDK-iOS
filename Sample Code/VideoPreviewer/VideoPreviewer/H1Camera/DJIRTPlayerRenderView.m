...
@property (nonatomic) MovieGLView* glView;
...
-(id) initWithDecoderType:(LiveStreamDecodeType)decodeType
    ...
    if(self = [super init]){
        ...
        _glView = [[MovieGLView alloc] initWithFrame:frame multiThreadSupported:YES];
        ...
        if(decodeType == LiveStreamDecodeType_Software){
            ...
            _softDecoder = [[SoftwareDecodeProcessor alloc] initWithExtractor:_frameExtractor];
...
-(void) reDrawWithGrayout:(BOOL)gray{
    ...
    //a hack, 如果不先初始化一下再redraw, 会出现问题
    //可能跟OpenGL ES的多线程特性有关, 但是没有定位出具体原因
    if(_framePushed == NO)
        return;
